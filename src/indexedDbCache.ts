...
class ArchiveCache {
  ...
  async save(
    key: string,
    files: Array<{ path: string; content: Uint8Array; size: number }>,
  ): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log(
          `IndexedDB save took ${duration.toFixed(2)}ms for ${files.length} files`,
        )
        resolve()
      }
    })
  ...
  async get(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log(
          `IndexedDB get took ${duration.toFixed(2)}ms ${files ? `for ${files.length} files` : "(cache miss)"}`,
        )
        resolve(files)
      }
    })
